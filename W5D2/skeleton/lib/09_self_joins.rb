...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT
    mid.num, mid.company, stopc.name, c.num, c.company
  FROM
    (SELECT
      stopb.name, b.company, b.num, b.stop_id
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    ...
    JOIN
      stops stopb ON (b.stop_id = stopb.id)
    WHERE
      stopa.name = 'Craiglockhart') as mid 
  JOIN
    routes c ON (mid.company = c.company AND mid.num = c.num)
  JOIN
    stops stopmid ON (mid.stop_id = stopmid.id)
  JOIN
